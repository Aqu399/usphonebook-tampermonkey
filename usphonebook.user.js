...
// @namespace    uspb
// @version      13.0
// @updateURL    https://raw.githubusercontent.com/aquaqu0102/usphonebook-tampermonkey/main/usphonebook.user.js
// @downloadURL  https://raw.githubusercontent.com/aquaqu0102/usphonebook-tampermonkey/main/usphonebook.user.js
...
function createPanel(){
    ...
    p.addEventListener('click',function(e){
        ...
        if(btn.id==='b-go')run();
        else if(btn.id==='b-sp'){sv({_stop:true,results});stopF=true;l('⏹ 已停止,结果已保存','warn');}
        else if(btn.id==='b-csv')exp();
        else if(btn.id==='b-cl'){results=[];document.getElementById('uspb-fd').textContent='结果:0';if(rt)rt.textContent='';lg.innerHTML='';l('🗑','info')}
    });

    const state=ld();
    if(state&&state._stop){cl();l('⏹ 已停止,可重新开始');return}
    if(state){
